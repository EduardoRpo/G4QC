...
services:
  postgres:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U g4qc"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  ibgateway:
    ...
    environment:
      - IB_USERNAME=jgomez236
      - IB_PASSWORD=juan2306
      - IB_REGION=America
